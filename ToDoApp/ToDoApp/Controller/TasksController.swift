...
class TasksController: UITableViewController {
    ...
    @IBAction func addTasks(_ sender: UIBarButtonItem) {
        // set up alert
        let controllerAlert = UIAlertController(title: "Add A New Task", message: nil, preferredStyle: UIAlertControllerStyle.alert)
        
        // set up all actions
       // let actionAdd = UIAlertAction(title: "Add", style: UIAlertActionStyle.default, handler: nil)
        let actionAdd = UIAlertAction(title: "Add", style: UIAlertActionStyle.default){ action in
            // get text
        ...
        controllerAlert.addTextField { textField in
            ...
            textField.addTarget(self, action: #selector(self.textFieldHandler), for: UIControlEvents.editingChanged)
        }
...
extension TasksController{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // set up alert
        let controllerAlert = UIAlertController(title: "Alter The Task", message: nil, preferredStyle: UIAlertControllerStyle.alert)
        
        // set up all actions
     
        let actionEdit = UIAlertAction(title: "Edit", style: UIAlertActionStyle.default){ action in
            // get text
            // there is only one textfiled in the box
            // so it is the first one
            guard let text = controllerAlert.textFields?.first?.text
    ...
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let action = UIContextualAction(style: UIContextualAction.Style.normal, title: nil) { (action, sourceView, completionHandler) in
               ...
               // print(self.taskStore.tasks[indexPath.section][indexPath.row].name)
                ...
                // insert row to view
                tableView.insertRows(at: [IndexPath(row: 0, section: 1)], with: UITableViewRowAnimation.automatic)
